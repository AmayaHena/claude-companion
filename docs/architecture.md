...
deeper and never match. `Recent(projectsDir, n)` lists the same files newest
first with the first entry's cwd and the first prompt, for the picker shown
...
than `width`, plus one blank line. A header row is the clock, a space, the
glyph and title; body rows are six spaces and the two-cell accent bar (8
cells of gutter either way). The bar has one hue per action type: yellow
command, red failed command, magenta edit, cyan creation. `Marks(cmd)`
classifies a command: a git or GitHub write gets the
...
indices, so the terminal theme decides the shades.
...
so the screen shows the work since the last message. The view follows the
tail while the user is at the bottom and stops when they scroll up. The log
shows from the first line; there is no opening screen.
